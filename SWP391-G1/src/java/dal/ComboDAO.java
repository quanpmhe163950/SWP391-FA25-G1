...
public class ComboDAO extends DBContext {
    ...
    // Trả về List tất cả combos mới nhất
    ...
    public boolean addCombo(Combo c) {
        String sql = "INSERT INTO Combo (ComboName, Description, Price, ImagePath, Status) VALUES (?, ?, ?, ?, ?)";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
    ...
    public boolean updateCombo(Combo c) {
        String sql = "UPDATE Combo SET ComboName=?, Description=?, Price=?, ImagePath=?, Status=? WHERE ComboID=?";
        try (PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, c.getComboName());
            ps.setString(2, c.getDescription());
            ps.setDouble(3, c.getPrice());
